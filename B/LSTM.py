...
def Input_convert(x_train, x_test, y_train, y_test):
    ...
    yy_train = np.zeros([y_train.shape[0],2])
    for n in range(y_train.shape[0]):
        if (y_train[n] == 0):yy_train[n] = [1,0]
        if (y_train[n] == 1):yy_train[n] = [0,1]
    yy_test = np.zeros([y_test.shape[0],2])
    for n in range(y_test.shape[0]):
        if (y_test[n] == 0):yy_test[n] = [1,0]
        if (y_test[n] == 1):yy_test[n] = [0,1]
...
def LSTM_modeling(x_train, x_test, y_train, y_test):
    ...
    model.add(Dropout(0.2))  # dropout layer
    model.add(Dense(2, activation='softmax'))  #dense (output) layer 
